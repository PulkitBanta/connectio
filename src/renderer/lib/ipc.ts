...
export const onLog = (cb: (entry: { method: string; path: string; status: number; ms: number; targetUrl: string }) => void): void =>
  window.connectio.onLog(cb);
...
export const config = {
  ...
  delete: (name: string): Promise<{ ok: boolean }> =>
    window.connectio.config.delete(name),
